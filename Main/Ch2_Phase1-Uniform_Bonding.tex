...
\begin{equation}\tag{B} \label{eq:radius_indium}
    \begin{split}
...
\begin{equation}\tag{B} \label{eq:initial_radius}
    \begin{split}
        \sqrt{\frac{h_{Initial}}{ 0.65 \um}} r_{Initial} &= r_{Final} \\
...
The Handbook of Wafer Bonding \cite{waferBondingHandbook} suggests that a typical $\ch{In-Au}$ bump consists of a $3\um$ thick indium layer and a $0.3\um$ thick gold layer. The diagram in \ref{fig:waferbondingdiagram} part (a) shows what a typical construction of a microbump looks like. While there are differences in the way we construct our bond which looks like \ref{fig:bond_diagram}
